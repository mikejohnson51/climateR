...
makeGrid = function(r){
  ...
  e = cbind(c(min(lon), max(lon) , min(lon)), c(min(lat), max(lat), min(lat))) %>%
    list() %>%
    sf::st_polygon() %>%
    sf::st_sfc() %>%
    sf::st_sf()
  ...
  ids = matrix(c(1:(x.dim*y.dim)),
               nrow = y.dim,
               ncol = x.dim,
               byrow = F)

  df = data.frame(d = raster::values(raster::t(r)), grid_ids = matrix(ids, ncol = 1))
  names(df) = c(names(r), "grid_ids")

  grd_lrg = sf::st_make_grid(e, n = c(x.dim, y.dim)) %>%
    sf::st_set_crs(as.character(r@crs)) %>%
    sf::st_sf()

  grd_lrg[['grid_ids']] = as.vector(t(apply(ids, 2, rev)))

  return(merge(grd_lrg, df, "grid_ids"))
}
