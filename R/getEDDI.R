...
getEDDI =  function(AOI = NULL, startDate, endDate = NULL,  timestep = "1 week") {
  ...
  url <- paste0(base, d$year, "/", "EDDI_ETrs_", num, abb, "_", d$string, ".asc")

  s = raster::stack()

  for (i in seq_along(url)) {
      dest = tempfile(pattern = basename(url[i]))
      suppressWarnings( httr::GET(url[i], httr::write_disk(dest, overwrite = TRUE)) ) # suppressing Government Warning
